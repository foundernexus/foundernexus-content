...
const DEAD = ['/apply', '/nominate', '/newsletter'];
    ...
    while ((m = linkRe.exec(text))) {
      ...
      if (/^\//.test(target)) {
        if (DEAD.includes(target))
          out.push(`${relp}: links to ${target} (known 404 route — not a live CTA yet)`);
        continue;
      }
      const base = /^\.\.?\//.test(target) ? resolve(dirname(f), target) : resolve(ROOT, target);
      if (!existsSync(base)) out.push(`${relp}: internal link target not found: ${target}`);
    }
    for (const d of DEAD) {
      if (new RegExp('(^|\\s)' + d + '(\\s|$)', 'm').test(text))
        out.push(`${relp}: mentions ${d} (known 404 route today; confirm live 200 before publishing)`);
    }
  }
...
try {
  ...
  const files = arg ? [resolve(ROOT, arg)] : listFiles(['content', 'social', 'briefs'], ['.md']);
