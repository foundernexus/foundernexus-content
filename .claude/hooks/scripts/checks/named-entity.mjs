...
function hasSignoff(text) {
  return /review_flag:|status:\s*held_review|signed[-_ ]?off|court\s*\/\s*legal|legal sign-?off/i.test(text);
}
...
try {
  ...
  process.stderr.write('WARN named-entity could not run: ' + (e && e.message) + '\n');
  process.exit(3);
}
